...
async def partner_summary(
    ...
    # Calculate today's revenue from paid payments
    # Get all payments that were paid today (status = PAID and paid_at is today)
    ...
    def maybe_warn(limit: Optional[int], usage: int, label: str) -> None:
        if limit is None:
            return
